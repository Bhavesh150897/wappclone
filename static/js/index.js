const app = new Vue({
  ...
  methods: {
    ...
    async placeCall() {
        const userOnlineChannel = pusher.subscribe("presence-online-channel");
        const dataaa = userOnlineChannel['members']['members'];
        const rec_keys_user = Object.keys(dataaa)[0];
        const rec_keys_rec = Object.keys(dataaa)[1];
        const channelName = '';
        const tokenRes = '';

      try {
        if (AUTH_USER_ID == rec_keys_user) {
            const channelName = `${AUTH_USER_ID}_${rec_keys_rec}`;
            const tokenRes = await this.generateToken(channelName);

            let placeCallRes = await axios.post(
              "/call-user/",
              {
                user_to_call: rec_keys_rec,
            ...
            const channelName = `${AUTH_USER_ID}_${rec_keys_user}`;
            const tokenRes = await this.generateToken(channelName);

            let placeCallRes = await axios.post(
              "/call-user/",
              {
                user_to_call: rec_keys_user,
                channel_name: channelName,
              },
              {
                headers: {
